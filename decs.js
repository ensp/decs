	...
	jQuery(document).ready(function(){
		...
		if(jQuery("input[id*='decs-module']").length == 1){ // inicializa apenas quando o campo do DeCS for carregado  
			...
			Decs.html = '' + // HTML da interface do modulo DeCS
						...
						'	<div id="decs-module-panel-toggle" class="decs-module-panel-toggle-closed">Selecionar Descritores</div>' + // decs-module-panel-toggle																
						...
						'			<input type="button" id="decs-module-search-button" value="Procurar"/>' + // decs-module-search-button
						'		</div>' +
						...
						'			<li id="decs-module-selected-descriptors-list-title">nenhum descritor selecionado</li>' + // decs-module-selected-descriptors-list-title
						'		</ul>' +									
						'	</div>' +										
			...
			Decs.updateInvisibleField = function(descriptor){			
				...
				if (descriptors.split(' ').join('') == ''){ // se o campo DeCS estiver vazio								
					...
					alert("\"" + descriptor + "\" já foi selecionado"); // alerta se houver duplicidade				
				}	
			};			
			...
			Decs.updateDescriptors = function(descriptors){					
				...
				if(html == '') // mostra total de descritores 
					html = '<li class="decs-module-alert-1">Termo pesquisado: <em>' + termo + '</em> | Descritores encontrados: <em>nenhum</em></li>' + html;
				else
					html = '<li class="decs-module-alert-2">Termo pesquisado: <em>' + termo + '</em> | Descritores encontrados: <em>' + total + '</em></li>' + html;						
